...
## Final Thoughts
Overall, this project was simultaneously an absolute blast and my own personal "invalid geometry" hell.  It was super fun to try to find a single mathematical process that solved such a complicated problem, and extremely satisfying to watch the program run in real-time and work through emergent complexities of the real-life implementation.  This project really threw me in the deep end of manipulating complex geometries at scale, and taught me a ton about efficiently managing expensive geometry calculations and implementing performance optimization shortcuts.  At the same time, there were a *ton* of headaches dealing with invalid/misaligned precinct geometries, troubleshooting confusing edge cases, doom-tuning model hyperparameters, and iterating the overall approach.  In terms of where I would take this project next or things I would do differently:

- **More Extensive Data Cleansing**: In retrospect, a lot of headaches could have been avoided by more thoroughly cleaning up the VEST precinct geometries at the start.  Obviously, the map isn't perfect, and I didn't fully grasp how much downstream pain would result from tiny gaps between precinct geometries, minute geometry overlaps, and fractured precinct geometries.  A lot of patches had to be added to the scripts to prevent the contiguity flags from contantly firing, and I suspect a lot of this could have been avoided by getting to a cleaner state mapping before starting.
- **Risky Allocation of Non-Geographic Votes**:  Early on in the data cleansing, I had to allocate non-geographic mail-in and advance polling votes to physical day-of precincts.  While I don't think it was avoidable with the data I had, I suspect that this misrepresented the true geographic spread of voters.  In reality, a *majority* of the votes cast were allocated to non-geographic precincts, and so assumptions about the geographic dispersion of the majority were actually made based on the minority.  If you look at the precinct heatmap I calcualated and zoom into the original District 1: you can see that voter demographics starkly change as soon as you cross district lines... I suspect that in reality Demographic representation within this district was far more localized, and my approach distriubted mail-in Democratic support too broadly across the entire district.
- **Investigating Adjency Graphs**: For maintaining contiguity, this whole approach relied on a framework of expensive union-border-intersects evaluations.  In my later research, I came across the concept of pre-defining an adjacency graph of neighbouring precincts, which I suspect could have greatly expedited the runtime and avoided a lot of the invalid union geometry headaches.
